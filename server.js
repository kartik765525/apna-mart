...
function getISTDateKeyFromDate(date = new Date()) {
...
function getOrderDateKey(order) {
  const raw = order.deliveredAtISO || order.createdAtISO || order.createdAt || order.date || order.deliveredAt || '';

  if (raw && !String(raw).includes('/')) {
    const d = new Date(raw);
    if (!Number.isNaN(d.getTime())) return getISTDateKeyFromDate(d);
  }

  const m = String(raw).match(/(\d{2})\/(\d{2})\/(\d{4})/);
  if (m) return `${m[3]}-${m[2]}-${m[1]}`;

  return getISTDateKeyFromDate();
}

function isTodayOrder(order) {
  return getOrderDateKey(order) === getISTDateKeyFromDate();
}
...
async function cleanupOldDeliveredOrders() {
  ...
  if (filtered.length !== orders.length) {
    ...
    console.log(`Old delivered orders deleted: ${orders.length - filtered.length}`);
  }

  return filtered;
}

function startDeliveredOrdersMidnightCleanup() {
  let lastCleanupDate = '';

  setInterval(async () => {
    try {
      const now = new Date();
      const istDate = getISTDateKeyFromDate(now);
      ...
      if (istTime === '00:00' && lastCleanupDate !== istDate) {
        lastCleanupDate = istDate;
        await cleanupOldDeliveredOrders();
      }
    } catch (error) {
      console.error('Delivered orders cleanup error:', error.message);
    }
  }, 60 * 1000);
}
...
app.post('/api/orders', requireCustomer, async (req, res) => {
  ...
  const { name, address, paymentMethod, items, couponCode } = req.body;
  ...
  const cleanCustomerLocation = null;
  ...
  const newOrder = {
    ...
    deliveryStartedAt: '',
    deliveredAt: '',
    customerLocation: clearLiveLocation(),
    date: getISTDateTime(),
...
app.patch('/api/customer/location', requireCustomer, async (req, res) => {
  return res.json({
    message: 'Customer live location disabled',
    updatedCount: 0
  });
});
...
app.patch('/api/admin/orders/:id/status', requireAdmin, async (req, res) => {
  ...
  if (status) {
    ...
    if (status === 'Delivered') {
      ...
      orders[index].deliveredAtISO = getNowISO();
...
app.get('/api/delivery/orders', requireDelivery, async (req, res) => {
  const orders = await cleanupOldDeliveredOrders();
  ...
  if (deliveryId === 'DB-DEMO-DELIVERY') {
    deliveryOrders = orders.filter((order) => {
      ...
      return ['Pending', 'Out for Delivery', 'Delivered'].includes(status) && isTodayOrder(order);
    });
  } else {
    deliveryOrders = orders.filter((order) => {
      const assignedId = String(order.assignedDeliveryBoyId || order.deliveryBoyId || '').trim();
      return assignedId === deliveryId && isTodayOrder(order);
    });
  }
...
startDeliveredOrdersMidnightCleanup();
cleanupOldDeliveredOrders().catch((error) => {
  console.error('Startup delivered cleanup error:', error.message);
});
